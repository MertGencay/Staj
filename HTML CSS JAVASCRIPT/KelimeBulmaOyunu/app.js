...
// 5 harfli kelimelerden oluşan liste (sen buraya kendi kelimelerini koyabilirsin)
const wordList = [
  "körpe",
		  "acele",
		  "güneş",
...
function toUpperCaseTR(str) {
    ...
    return str.replace(/[\iışğüöç]/g, char => map[char] || char.toUpperCase());
}
...
function handleKeyClick(key) {
    ...
    if (key === "←") {
        ...
        if (guessComplete) {
            let guessWord = "";
            for (let c = 0; c < width; c++) {
                guessWord += document.getElementById(row + "-" + c).innerText;
            }

            if (!wordList.includes(guessWord.toLowerCase())) {
    ...
    document.addEventListener("keyup", (e) => {
        ...
        if (e.key.length === 1 && /^[a-zA-ZçğıöşüÇĞİÖŞÜ]$/.test(e.key)){
            if (col < width){
                ...
                if (currTile.innerText == ""){
                    currTile.innerText = toUpperCaseTR(e.key);
                    col += 1;
                }
            }
        }
            ...
            if (!wordList.includes(guessWord.toLowerCase())) {
                alert("Kelime listede yok! Lütfen geçerli bir kelime girin.");
                return;
            }
        ...
        if (!gameOver && row == height){
            gameOver = true;
            document.getElementById("answer").innerText = "Doğru kelime: " + word;
        }
    })
}
...
function update(){
    ...
    keys.forEach((keyBtn) => {
        if (keyBtn.innerText.toUpperCase() === letter) {
            if (status === "correct") {
